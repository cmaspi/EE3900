...
\begin{frame}
    \begin{proof}
        \only<1>{Let $\vec{Ax}=\vec{b}$ have at least one solution
        \begin{align}
            \vec{Ax}&=\vec{b}\\
        ...
        }\only<2>{The entire set of solution is given by $\vec{A}^+\vec{b}+\vec{k}$, where $\vec{k}$ is a vector in kernel space or null space of $\vec{A}$
        \begin{align}
        ...
        }\only<3>{Now, we will prove that the above equation holds true
        \begin{align}
